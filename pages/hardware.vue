<template>
    <NuxtImg provider="iGEM" src="/banner/drylab.webp" class="inline-block w-full mt-[-1rem]" alt="cover"/>
    <div class="page-title m-10 mb-16 font-normal text-3xl p-5 pl-8 rounded-lg">Hardware</div>
    <div class="flex flex-row flex-wrap md:flex-nowrap justify-center mx-10">
        <ContentDoc path="/drylab-hardware" v-slot="{ doc }">
            <!-- TOC -->
            <aside class="md:sticky md:top-[18vh] md:w-[200px] md:h-[500px] md:mr-10 ">
                <h1 class="mt-0">TOC</h1>
                <ul>
                    <li v-for="link in doc.body.toc.links" :key="link.id" class="my-4"
...
.page-title {
    background: linear-gradient(90deg, rgba(181, 194, 190, 0.8) 0%, rgba(181, 194, 190, 0) 100%);
}
</style>
