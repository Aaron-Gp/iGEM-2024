...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="page-title m-10 mb-16 font-normal text-3xl p-5 pl-8 rounded-lg">Engineering</div><div class="flex flex-row flex-wrap md:flex-nowrap justify-center mx-10">`);
      _push(ssrRenderComponent(_component_ContentDoc, { path: "/project-engineering" }, {
        default: withCtx(({ doc }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<aside class="md:sticky md:top-[120px] md:w-[200px] md:h-[200px] md:mr-10"${_scopeId}><h1 class="mt-0"${_scopeId}>TOC</h1><ul${_scopeId}><!--[-->`);
            ssrRenderList(doc.body.toc.links, (link) => {
              _push2(`<li class="${ssrRenderClass([{ "text-[#C0F748] font-bold": unref(activateIndex) == link.id }, "my-5"])}"${_scopeId}><a class="cursor-pointer"${_scopeId}>${ssrInterpolate(link.text)}</a><ul class="${ssrRenderClass([{ "hidden": unref(activateIndex) != link.id }, "pl-5 pt-2"])}"${_scopeId}><!--[-->`);
              ssrRenderList(link.children, (sublink) => {
                _push2(`<!--[-->`);
                if (sublink.depth == 3) {
                  _push2(`<li class="${ssrRenderClass([{ "text-[#fff] font-normal": unref(activateSubIndex) != sublink.id }, "mb-2"])}"${_scopeId}><a class="cursor-pointer"${_scopeId}>${ssrInterpolate(sublink.text)}</a></li>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`<!--]-->`);
            ...
            return [
              createVNode("aside", { class: "md:sticky md:top-[120px] md:w-[200px] md:h-[200px] md:mr-10" }, [
...
//# sourceMappingURL=engineering-CvgHhXaL.mjs.map
